/**
 * Admin Blog Post Form — slug generation, featured image preview, TinyMCE.
 */
(function (window) {
    'use strict';
    ...
    function initFeaturedImageDelete(root) {
        var scope = root || document;
        var form = scope.querySelector('[data-blog-post-form]');
        var deleteBtn = scope.querySelector('#deleteFeaturedImageBtn');
        if (!deleteBtn || !form) return;

        deleteBtn.addEventListener('click', function () {
            if (!confirm('هل أنت متأكد من حذف الصورة؟')) return;

            var url = form.getAttribute('data-delete-featured-image-url');
            if (!url) return;
            ...
            fetch(url, {
                ...
                .then(function (response) {
                    ...
                    var container = scope.querySelector('#featuredImageContainer');
                    ...
                    var btnText = scope.querySelector('#featuredImageBtnText');
                    if (btnText) btnText.textContent = 'اختر صورة';
                })
                .catch(function () {
    ...
    function bindFormSubmit(form) {
        if (!form || form.dataset.blogPostSubmitBound) return;
        form.dataset.blogPostSubmitBound = '1';
    ...
    window.AdminBlogPostForm = {
        init: function (options) {
            ...
            initFeaturedImageDelete(root);
            bindFormSubmit(root.querySelector('[data-blog-post-form]'));
